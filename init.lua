...
-- Toggle grammar/spell checking (ltex LSP) — off by default; see ltex config in `servers`
...
-- Force spaces for TypeScript/JavaScript files
vim.api.nvim_create_autocmd('FileType', {
...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      local servers = {
        ...
        -- Grammar and spell checking for prose (off by default; toggle with <leader>tg)
        ltex = {
          autostart = false,
          settings = {
            ltex = {
  ...
  -- Smart commenting with visual selection support
  {
